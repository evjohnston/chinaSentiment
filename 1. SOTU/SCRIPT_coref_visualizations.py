...
# === Setup folders ===
base_folder = "1. SOTU"
csv_folder = os.path.join(base_folder, "SOTU_CSVs")
visuals_folder = os.path.join(base_folder, "SOTU_Charts/Vader Charts")
os.makedirs(visuals_folder, exist_ok=True)

# === Load summary and detailed data ===
summary_file = os.path.join(csv_folder, "vader_china_sentiment_summary.csv")
detailed_file = os.path.join(csv_folder, "vader_china_sentences_detailed.csv")
...
agg_sentiment = df.groupby(["president", "year"], as_index=False)["china_sentiment"].mean().sort_values("year")
...
agg_mentions = df.groupby(["president", "year"], as_index=False)["china_mention_count"].sum().sort_values("year")
plt.figure(figsize=(12, 6))
...
agg_recent_sentiment = df_recent.groupby(["president", "year"], as_index=False)["china_sentiment"].mean().sort_values("year")
plt.figure(figsize=(12, 6))
...
agg_recent_mentions = df_recent.groupby(["president", "year"], as_index=False)["china_mention_count"].sum().sort_values("year")
plt.figure(figsize=(12, 6))
